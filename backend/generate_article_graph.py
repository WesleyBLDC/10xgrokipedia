...
def generate_graph() -> Dict:
    ...
    # Build article index by UUID and by title (for internal link resolution)
    articles_by_id = {}
    articles_by_title = {}
    for article in articles:
        article_id = article.get('id')
        if not article_id:
            print(f"Warning: Article '{article.get('title', 'Unknown')}' has no UUID, skipping")
            continue
        articles_by_id[article_id] = article
        articles_by_title[article.get('title', '')] = article
    
    article_ids = list(articles_by_id.keys())
    print(f"Processing {len(article_ids)} articles with UUIDs")
    ...
    for article_id, article in articles_by_id.items():
        ...
        # Extract internal links (these are still by title, need to resolve to UUIDs)
        internal_link_titles = extract_internal_links(content)
        # Resolve internal links to UUIDs
        internal_link_ids = set()
        for link_title in internal_link_titles:
            # Try to find article by title
            linked_article = articles_by_title.get(link_title)
            if linked_article and linked_article.get('id'):
                internal_link_ids.add(linked_article['id'])
    ...
    for id1, id2 in combinations(article_ids, 2):
        data1 = article_data[id1]
        data2 = article_data[id2]
        
        shared_citations = data1['citations'] & data2['citations']
        if shared_citations:
            weight = WEIGHTS['shared_exact_citations'] * (len(shared_citations) / 5.0)  # Normalize
            edge_weights[(id1, id2)] += weight
            edge_types[(id1, id2)].add('shared_exact_citations')
            if 'shared_citations' not in edge_metadata[(id1, id2)]:
                edge_metadata[(id1, id2)]['shared_citations'] = []
            edge_metadata[(id1, id2)]['shared_citations'].extend(list(shared_citations))
    ...
    for id1, id2 in combinations(article_ids, 2):
        ...
        if shared_domains:
            ...
            domain_sharing[(id1, id2)] = count
            max_shared_domains = max(max_shared_domains, count)
    
    # Normalize and add weights
    for (id1, id2), count in domain_sharing.items():
        if max_shared_domains > 0:
            weight = WEIGHTS['shared_domains'] * (count / max_shared_domains)
            edge_weights[(id1, id2)] += weight
            edge_types[(id1, id2)].add('shared_domains')
            if 'shared_domains' not in edge_metadata[(id1, id2)]:
                edge_metadata[(id1, id2)]['shared_domains'] = []
            data1 = article_data[id1]
            data2 = article_data[id2]
            shared_domains = data1['citation_domains'] & data2['citation_domains']
            edge_metadata[(id1, id2)]['shared_domains'] = list(shared_domains)
        ...
        if domains1 or domains2:
            ...
            if union > 0:
                ...
                if jaccard > 0:
                    ...
                    edge_weights[(id1, id2)] += weight
                    edge_types[(id1, id2)].add('domain_jaccard')
    
    # 2. Internal link edges
    print("  - Processing internal link edges...")
    
    # Direct links
    for id1, data1 in article_data.items():
        for linked_id in data1['internal_links']:
            if linked_id in articles_by_id:
                # A -> B
                edge_key = (id1, linked_id)
                ...
                edge_metadata[edge_key]['link_direction'].append(f"{id1}->{linked_id}")
                
                # Check if bidirectional
                data2 = article_data.get(linked_id, {})
                if id1 in data2.get('internal_links', set()):
                    edge_weights[edge_key] += WEIGHTS['bidirectional_link']
                    edge_types[edge_key].add('bidirectional_link')
                    edge_metadata[edge_key]['link_direction'].append(f"{linked_id}->{id1}")
    
    # Co-linked articles
    link_targets = defaultdict(set)
    for article_id, data in article_data.items():
        for target_id in data['internal_links']:
            if target_id in articles_by_id:
                link_targets[target_id].add(article_id)
    
    for target_id, sources in link_targets.items():
        if len(sources) > 1:
            for source1, source2 in combinations(sources, 2):
                edge_key = (source1, source2)
                weight = WEIGHTS['co_linked'] * (len(link_targets[target_id]) ** 0.5)
                ...
                edge_metadata[edge_key]['co_linked_targets'].append(target_id)
    ...
    for article_id, article in articles_by_id.items():
        ...
        nodes.append({
            'id': article_id,  # Use UUID as node ID
            'title': data['title'],
